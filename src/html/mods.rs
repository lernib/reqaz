...
/// The component internal mod, which resolves
/// any `object` tags with props and applies them
/// to the underlying `href` content. More advanced
/// version of `fetch`.
mod component;
...
impl HtmlModManager {
    ...
    fn load_mod(&mut self, mod_name: &str) -> Option<Box<dyn HtmlMod>> {
        let mod_box: Box<dyn HtmlMod> = match mod_name {
            "fetch" => Box::new(fetch::Mod::new(
                self.page_uri.clone(),
                self.resolver.clone(),
            )),
            "css" => Box::<css::Mod>::default(),
            "component" => Box::new(component::Mod::new(
                self.page_uri.clone(),
                self.resolver.clone(),
            )),
            _ => return None,
    ...
    pub fn load_mods<const N: usize>(&mut self, mods: [&str; N]) {
        for mod_name in mods {
            if let Some(mod_) = self.load_mod(mod_name) {
                self.mod_cache.insert(mod_name.to_owned(), mod_);
            }
        }
    }
